...
def solve_reynolds_2d_compressible(
    h,
    dx,
    dy,
    u_l,
    ...
    """
    Very simple 2D compressible Reynolds solver with ideal gas ρ(p).

    We iterate:
      1. given p^k, compute ρ^k = ρ_ref * p^k / p_ref
      2. solve linearised Reynolds for p^{k+1}

    BC in x: p = p_ref.
    BC in y: p_ref (dirichlet) or Neumann (copy interior).
    """
    h = np.asarray(h)
    Ny, Nx = h.shape

    # Initial guess: uniform p_ref
